...
public class NTriples implements Serializer {
		...
		try{
			...
			if(nrOfOutputFiles==1)
				...
				for(int i=1;i<=nrOfOutputFiles;i++)
					if (i > (Generator.fromFileId-1) && i < (Generator.toFileId+1)){
	...
	public void gatherData(ObjectBundle bundle) {
		...
		while(it.hasNext())
		{
			...
			if (currentWriter > (Generator.fromFileId-2) && currentWriter < Generator.toFileId){
				try{
	...
	public void serialize() {
		...
		try {
			for(int i=0;i<fileWriter.length;i++) {
				if (i > (Generator.fromFileId-2) && i < Generator.toFileId){
					fileWriter[i].flush();
					fileWriter[i].close();
				}
			}
