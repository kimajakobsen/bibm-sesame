...
public class NTriplesGZ implements Serializer {
	private Writer[] fileWriter;
	private boolean forwardChaining;
	private long nrTriples;
	private int currentWriter = 0;
	
	public NTriplesGZ(String file, boolean forwardChaining)
	{
		this(file, forwardChaining, 1);
	}
	
	public NTriplesGZ(String file, boolean forwardChaining, int nrOfOutputFiles)
	{
		int nrOfDigits = ((int)Math.log10(nrOfOutputFiles)) + 1;
		String formatString = "%0" + nrOfDigits + "d";
		try{
			fileWriter = new Writer[nrOfOutputFiles];
			if(nrOfOutputFiles==1)
			        fileWriter[0] =
				    new BufferedWriter(new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(file + ".nt.gz"))));
			else
				for(int i=1;i<=nrOfOutputFiles;i++)
					fileWriter[i-1] =
					    new BufferedWriter( new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(file + String.format(formatString, i) + ".nt.gz"))));
		} catch(IOException e){
