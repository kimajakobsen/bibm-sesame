...
public class TurtleGZ implements Serializer {
	...
	public TurtleGZ(String file, boolean forwardChaining)
	{
		this(file, forwardChaining, 1);
	}
	
	public TurtleGZ(String file, boolean forwardChaining, int nrOfOutputFiles)
	{
		int nrOfDigits = ((int)Math.log10(nrOfOutputFiles)) + 1;
		String formatString = "%0" + nrOfDigits + "d";
		try{
			dataFileWriter = new Writer[nrOfOutputFiles];
			if(nrOfOutputFiles==1)
				this.dataFileWriter[0] =
				    new BufferedWriter(new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(file + ".ttl.gz"))));
			else
				for(int i=1;i<=nrOfOutputFiles;i++){
					if (Generator.noOfMachines != -1){
						if (i % Generator.noOfMachines == Generator.thisMachineId){
							dataFileWriter[i-1] =
							    new BufferedWriter( new OutputStreamWriter(new GZIPOutputStream(new FileOutputStream(file + String.format(formatString, i) + ".ttl.gz"))));
						}
					}
		...
		TurtleGZShutdown sd = new TurtleGZShutdown(this);
	...
	private void writePartOfPrefixes(StringBuilder sb){
		//System.out.println("Write out the stringBuilder ");
		try {
	...
	private void generatePrefixes() {
		...
		for(int i=1; i< Generator.producerOfProduct.size(); i++) {
			...
			sb.append(i);
			sb.append("/> .\n");
			if (sb.length() > 1000000000){	//write out and reset the string builder
				System.out.println("Write out long string in dataFromProducer Prefixes");
				writePartOfPrefixes(sb);
				sb.setLength(0);
			}
		}
		writePartOfPrefixes(sb);
		...
		for(int i=1; i< Generator.vendorOfOffer.size(); i++) {
			...
			if (sb.length() > 1000000000){	//write out and reset the string builder
				System.out.println("Write out long string in Vendor Prefixes");
				writePartOfPrefixes(sb);
				sb.setLength(0);
			}
		}
		...
		for(int i=1; i< Generator.ratingsiteOfReview.size(); i++) {
			...
			if (sb.length() > 1000000000){	//write out and reset the string builder
				System.out.println("Write out long string in Rating prefixes");
				writePartOfPrefixes(sb);
				sb.setLength(0);
			}
		}
	...
	public void gatherData(ObjectBundle bundle) {
		...
		try {
			while(it.hasNext())
			{
				...
				else if (currentWriter > (Generator.fromFileId-2) && currentWriter < Generator.toFileId){
					if(obj instanceof ProductType){
						dataFileWriter[currentWriter].append(convertProductType((ProductType)obj));
					}
					else if(obj instanceof Offer){
...
//	}
	...
	class TurtleGZShutdown extends Thread {
		TurtleGZ serializer;
		TurtleGZShutdown(TurtleGZ t) {
			serializer = t;
		}
